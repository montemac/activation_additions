...
for prompt in tqdm(prompts):
    for coeff in tqdm(coeffs):
        ...
        results_this = xvector.complete_prompt_with_x_vector(model=model, recipe=[(recipe_phrases, coeff)], 
            ...
            completion_length=50, layer_num=layer, temperature=temperature, freq_penalty=freq_penalty, top_p=top_p)
...
results['normal_sentiment_is_positive'] = results['normal_sentiment_label'] == 'POSITIVE'
results['patched_sentiment_is_positive'] = results['patched_sentiment_label'] == 'POSITIVE'

# Reduce over num_completions
grp = results.groupby(['prompt', 'coeff'])
positive_frace_rdu = grp[['normal_sentiment_is_positive', 'patched_sentiment_is_positive']].mean()
losses_rdu = grp[['normal_loss', 'patched_loss']].mean()

plot_df = pd.concat([positive_frace_rdu, losses_rdu], axis='columns').stack().reset_index()

px.line(plot_df, x='coeff', y=0, color='level_2', facet_col='prompt', log_x=True).show()
