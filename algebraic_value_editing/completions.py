...
def prompt_to_tokens(model: HookedTransformer, prompt: Union[str, List[str]]) -> torch.Tensor:
...
def complete_prompt_with_rich_prompts(
    ...
    **sampling_kwargs,  # Keyword arguments to pass to the model's generate function
    ...
    args:
        ...
        rich_prompts: A list of RichPrompts which will generate hook functions.
        tokens_to_generate: The number of additional tokens to generate.
        control_type: The type of control to use. Currently not implemented.
        seed: A random seed to use for generation.
        include_normal: Whether to include completions from the unmodified model.
        sampling_kwargs: Keyword arguments to pass to the model's generate function.

    returns:
        A DataFrame with the completions and losses. If include_normal is True, also includes
        completions and losses from the unmodified model.
    """
    ...
    patched_loss = (
        model(patched_completion.clone(), return_type="loss", loss_per_token=True)
        .mean(axis=1)
        .detach()
        .cpu()
        .numpy()
...
def print_n_comparisons(
    ...
    args:
        ...
        include_normal: Whether to include completions from the unmodified model.
        kwargs: Keyword arguments to pass to complete_prompt_with_rich_prompts.
    """
    # Generate the table
    table = prettytable.PrettyTable()
    table.align = "l"
    field_names = ["Patched completion"]
    if include_normal:
    ...
    width = 60
    table.min_width = table.max_width = width
    ...
    # Get the completions
    results = complete_prompt_with_rich_prompts(
        prompt=prompts_list, include_normal=include_normal, **kwargs
    )

    def apply_formatting(str_):
        completion = "".join(str_.split(prompt)[1:])  # Remove the prompt
