...
def get_x_vector_all_layers(
    model, x_vector_defs: List[Tuple[Tuple[str, str], float]], act_name: str
):
        ...
        if a_tokens.shape != b_tokens.shape:
            ...
            if len_diff > 0:  # Add to b_tokens
                b_tokens = torch.tensor(
                    b_tokens[0].tolist() + [SPACE_TOKEN] * abs(len_diff),
                    dtype=torch.int64,
                    device=model.cfg.device,
                ).unsqueeze(0)
            else:
                a_tokens = torch.tensor(
                    a_tokens[0].tolist() + [SPACE_TOKEN] * abs(len_diff),
                    dtype=torch.int64,
                    device=model.cfg.device,
                ).unsqueeze(0)
        assert (
            a_tokens.shape == b_tokens.shape
        ), f"Need same shape to compute an X-vector; instead, we have strA shape of {a_tokens.shape} and baseline shape of {b_tokens.shape}"

        # Run forward passes
        # TODO: do this in one call batching a/b together
        _, a_cache = model.run_with_cache(
            a_tokens, names_filter=lambda ss: ss == act_name
        )
        _, b_cache = model.run_with_cache(
            b_tokens, names_filter=lambda ss: ss == act_name
        )
...
def get_x_vector_fn(x_vector):
    ...
    def x_vector_hook_resize(
        resid_pre: Float[torch.Tensor, "batch pos d_model"], hook: HookPoint
        ...
        if x_vec_len > resid_pre.shape[-2]:
            return (
                resid_pre  # NOTE If computing last new set of vectors due to caching?
            )
        # NOTE this is going to fail when context window starts rolling over
        resid_pre[..., :x_vec_len, :] = (
            x_vector + resid_pre[..., :x_vec_len, :]
...
def complete_prompt_with_x_vector(
    ...
    **sampling_kwargs,
    ...
    normal_completion = model.generate(
        target_tokens,
        max_new_tokens=completion_length,
        verbose=False,
        **sampling_kwargs,
    ...
    results = pd.DataFrame(
        {
            "prompt": prompt,
            "normal_completion": [
                model.to_string(compl[1:]) for compl in normal_completion
            ],
            "patched_completion": [
    ...
    if return_loss:
        results["normal_loss"] = (
            model(normal_completion, return_type="loss", loss_per_token=True)
            .mean(axis=1)
            .detach()
            .cpu()
            .numpy()
        )
        results["patched_loss"] = (
...
def print_n_comparisons(num_comparisons: int = 5, **kwargs):
    """Pretty-print num_comparisons generations from patched and unpatched. Takes parameters for complete_prompt_with_x_vector."""
    ...
    table.align = "l"
    table.field_names = [
        f"\033[1mPatched completion\033[0m",
        f"\033[1mNormal completion\033[0m",
    ]
